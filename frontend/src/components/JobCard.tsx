...
export const JobCard: React.FC<JobCardProps> = ({
  ...
  const getMatchScoreBadge = () => {
    ...
    if (job.match_score === undefined || job.match_score === null) return null;
    ...
    return (
      ...
      >
        ...
        <span>{score > 0 ? `Match ${score}%` : 'Match Score'}</span>
      </button>
    );
  };
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-cyan-400 truncate">
                Source: {job.source_name || (job.source_id === 1 ? 'RemoteOK' : 'Public Source')}
              </span>
              {!job.is_active && (
